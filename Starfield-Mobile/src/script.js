...
(function($, window, document, undefined) {
    ...
    Starfield.prototype = {
        ...
        anim: function(){
            this.context.clearRect(0, 0, this.w, this.h); // Clear previous frame's stars

            this.context.fillStyle = 'rgba(0, 0, 0, 0.2)'; // Add a slight fade effect
...
if (isMobile) {
    function enableDeviceMotion() {
        window.addEventListener('deviceorientation', handleDeviceMotion, false);
    }
    ...
    function handleDeviceMotion(event) {
        ...
        let x = isPortrait ? event.gamma : event.beta * -1;
        let y = isPortrait ? event.beta : event.gamma;
        
        x = Math.min(Math.max(x, -90), 90);
        y = Math.min(Math.max(y, -90), 90);
        
        const scaledX = (x + 90) * (window.innerWidth / 180);
        const scaledY = (y + 90) * (window.innerHeight / 180);
        
        uniforms.mouse.value.x = scaledX;
        uniforms.mouse.value.y = renderer.domElement.height - scaledY;
        
        if (window.that) {
            that.cursor_x = scaledX;
            that.cursor_y = scaledY;
        }
    }
    ...
    function handleTouch(e) {
        ...
        uniforms.mouse.value.y = renderer.domElement.height - touch.clientY;
        
        if (window.that) {
            that.cursor_x = touch.clientX;
            that.cursor_y = touch.clientY;
        }
    }
