...
function init() {
  ...
  if (typeof DeviceOrientationEvent !== 'undefined' && typeof DeviceOrientationEvent.requestPermission === 'function') {
    // iOS 13+ devices require permission to access device orientation
    ...
    // Non-iOS devices or iOS versions prior to 13
    window.addEventListener('deviceorientation', handleOrientation);
    document.getElementById('requestPermissionButton').style.display = 'none';
  }
}
...
function handleOrientation(event) {
  ...
  const normalizedGamma = gamma / 90; // gamma is between -90 and 90
  const normalizedBeta = beta / 180;  // beta is between -180 to 180
